...
static void coap_example_client(void *p)
{
    ...
    while (1) {
        ...
        { /* PUT */
            static coap_uri_t put_uri = {};
            const char* server_uri = "coaps://coap.golioth.io/logs";
            const char* mediatype = "application/json";
            const char* put_data = "{\"level\":\"info\",\"module\":\"example\",\"msg\":\"test\"}";

            coap_pdu_t* put_request = coap_new_pdu(COAP_MESSAGE_CON, COAP_REQUEST_CODE_PUT, session);
            if (!put_request) {
            ...
            coap_split_uri((const uint8_t*)server_uri, strlen(server_uri), &put_uri);
            coap_add_option(put_request, COAP_OPTION_URI_PATH, put_uri.path.length, put_uri.path.s);
            coap_add_option(put_request, COAP_OPTION_CONTENT_TYPE, sizeof(mediatype)-1, (const uint8_t*)mediatype);
            coap_add_data(put_request, strlen(put_data), (unsigned char*)put_data);
            ...
            while (resp_wait) {
                ...
                if (result >= 0) {
                    if (result >= wait_ms) {
                        ESP_LOGE(TAG, "No response from server for PUT");
                        break;
                    } else {
                        wait_ms -= result;
                    }
